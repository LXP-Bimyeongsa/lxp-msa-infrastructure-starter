...
| P-17 | **탈퇴·차단 회원의 잔여 access token** | D-32에서 계정을 비활성화하고 세션을 끊어도, **이미 발행된 access token은 만료까지 유효**하다. JWT 검증이 상태를 보지 않기 때문이다. 노출 상한을 토큰 수명(300초)으로 줄여 두었다. 완전히 막으려면 상태를 보는 검증이 필요한데, 후보는 ① Keycloak token introspection(RFC 7662) — logout된 세션의 토큰을 inactive로 판정하므로 정확하지만 **매 요청 Keycloak 호출**이 붙는다 ② gateway가 회원 상태를 조회 후 짧게 캐시 — 호출을 줄이는 대신 캐시 TTL만큼 창이 남는다. **D-33으로는 해결되지 않는다** — D-33은 "gateway를 거쳤는가"를 증명할 뿐 "그 사용자가 아직 유효한가"는 보지 않는다 |
