...
| P-18 | **gRPC·스케줄러의 서비스 토큰** | D-33은 REST 경로(gateway → 4개 서비스)만 다뤘다. subscription → member gRPC 호출은 여전히 토큰 없이 오간다 — member-service의 gRPC 포트(9092)에 직접 접근하면 회원 정보를 조회할 수 있다. 서킷브레이커(D-17)가 얽혀 있어 실패 분류(`UNAUTHENTICATED`를 서킷 집계에 넣을지)를 함께 정해야 한다 |
