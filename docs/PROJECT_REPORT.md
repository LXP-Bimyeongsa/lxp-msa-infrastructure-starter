...
| 기록된 설계 결정 | 45건 (D-01 ~ D-45) |
| 미결 항목 | 20건 (P-01 ~ P-20, 이 중 10건 해결) |
...
### ⑨ 정상 동작이 정기 결제를 영구히 껐다 (D-41)

CI를 실제로 돌려보려고 "고칠 만한 것"을 찾다가 나온 건입니다.

`charge()`는 이미 청구된 회차를 만나면 PG를 부르지 않고 건너뜁니다. 여기까지는 맞습니다. 문제는 **그 결과를 `PgApproval.declined("DUPLICATE_CYCLE", false)`로 돌려준 것**입니다. 호출 측에서는 "분실·도난 카드" 같은 재시도 불가 거절과 모양이 같아서 `schedule.deactivate()`로 들어갑니다.

즉 **정상적인 멱등 스킵이 그 구독의 정기 결제를 영구히 껐습니다.** 로그는 `warn` 한 줄이라 조용히 끊깁니다.

이건 `BillingScheduler`가 **분산 락을 쓰지 않기로 한 근거를 무너뜨립니다.** 주석은 "다중 인스턴스가 같은 스케줄을 집어도 unique 제약이 막는다"고 단언하지만, 실제로 그 상황이 오면 나중에 집은 쪽이 스케줄을 죽였습니다.

**기존 테스트가 이 경로를 이미 밟고 있었습니다.** `sameCycleChargedOnlyOnce`가 다중 인스턴스 레이스를 그대로 재현하면서, Payment 행 개수만 검사하고 스케줄 상태는 보지 않았습니다. 단언 두 줄을 추가하니 바로 실패했습니다.

근본 원인은 **어휘**였습니다 — 내부 멱등 스킵을 PG 응답 타입으로 표현한 순간, 호출 측이 그것을 거절로 다루는 것이 자연스러워졌습니다.

### ⑩ 관측 도구 자체가 관측되지 않았다 (D-44)

세션 도중 Zipkin이 `OutOfMemoryError`로 죽었습니다. 인메모리 저장소 상한이 500000이고 샘플링이 1.0인 것이 겹쳤습니다.

**더 나쁜 것은 죽은 뒤였습니다.** 재시작 정책이 없어 그대로 죽어 있었고, 헬스체크도 없어 `docker ps`에 정상으로 보였습니다. **44분 동안 트레이스가 비어 있었는데 아무도 몰랐습니다.** 서비스 로그에는 span 전송 실패 스택트레이스만 쌓여 진짜 오류를 덮고 있었습니다.

### ⑪ 기본값이 요청의 절반을 죽이고 있었다 (D-43)

컨테이너를 재생성한 직후 gateway가 죽은 인스턴스로 보내 500이 났습니다. **"GET은 되는데 POST는 안 되는" 것처럼 보여** 애플리케이션 코드를 한참 뒤졌습니다.

원인은 `spring.cloud.consul.discovery.query-passing`의 **기본값이 `false`** 라는 것이었습니다. 디스커버리가 `critical` 인스턴스까지 돌려주고 로드밸런서가 죽은 것과 산 것을 번갈아 고릅니다.

죽은 인스턴스 하나를 등록하고 30회 호출해 **15 성공 / 15 실패**를 실측했습니다. 정확히 절반이라는 숫자가 라운드로빈이라는 증거였습니다.
...
| 항목 | 결과 |
|---|---|
| 컨테이너 기동 | 21개 상시 실행 정상 (init 잡 2개는 완료 후 종료) |
| 서비스 빌드 | 서비스 6개 + 공통 모듈 2개 전부 통과 |
| 단위 테스트 | 9개 파일 (사가·멱등·서킷·소유권·업로드·audience 검증) |
...
로컬 Jenkins가 GitHub를 5분 주기로 폴링합니다. 잡·플러그인·보안은 전부 JCasC로 코드에 있습니다.

| 빌드 | 트리거 | 대상 | 결과 |
|---|---|---|---|
| #4 | SCM 변경 | payment-service | 이미지 1개 |
| #5 | SCM 변경 | 서비스 5개 | 이미지 5개 |
| #6 | SCM 변경 | course-service | 이미지 1개 |
| #7·#8 | SCM 변경 | **공용 설정 변경 감지 → 전체** | **이미지 6개** |

`config-repo/`·`compose*`처럼 공용 파일이 바뀌면 어느 서비스가 영향받는지 알 수 없으므로 전체를 빌드합니다. 실제로 그렇게 도는 것을 확인했습니다.

### 안정화 세션에서 잡은 것 (D-41 ~ D-44)

| 항목 | 검증 방법 | 결과 |
|---|---|---|
| 중복 회차 스킵 (D-41) | 회차를 되돌려 재실행 | `active=1` 유지, 회차 2→3 진행 |
| 로그 원인 유실 | Keycloak·member-service 정지 | `Caused by: NoRouteToHostException` 등 원인 노출 |
| MinIO 오류 분류 (D-42) | 시크릿을 틀리게 주입 | `code=AccessDenied` → **409에서 503으로** |
| 죽은 인스턴스 라우팅 (D-43) | 죽은 인스턴스 등록 후 30회 호출 | **15/15 실패 → 30/30 성공** |
| Zipkin OOM (D-44) | 스스로 죽는 컨테이너로 정책 실증 | 3회 재시작 후 포기 확인 |
...
| 항목 | 내용 |
|---|---|
| PG 실연동 (P-01) | 연동 **경계는 완성**(D-39) — 목 PG가 HTTP를 타고 지연·거절코드를 돌려준다. 실키는 사업자등록이 있어야 발급되므로 코드로 풀 수 없다 |
| 배포 대상 (P-07) | 현재 로컬 도커. EC2+compose 약 3일, EKS 1~2주. **결정이 먼저 필요** |
| HA 구성 (P-14) | RabbitMQ·MongoDB는 단일 노드(Consul만 3노드). 컨테이너 +8개 규모라 별도 프로파일로 |
| 재시작 정책 (P-20) | Zipkin만 적용(D-44). 나머지 21개는 여전히 죽으면 그대로 죽어 있다 |
| Rate Limit | 초기 설계도에 있었으나 **미구현**. 필요 시점에 Redis 기반으로 |
...
**검사하지 않은 것은 통과한 것처럼 보입니다.** 이 프로젝트에서 가장 여러 번 나온 패턴입니다.

| 사례 | 어떻게 숨어 있었나 |
|---|---|
| gRPC 테스트 포트 충돌 | 그때 포트가 우연히 비어 있어 통과 |
| mongo 초기화 레이스 | 이전 재기동에서 우연히 통과 |
| 테스트가 config-server에 의존 | 컨테이너가 떠 있어 설정 누락이 안 드러남 |
| Docker 이미지 빌드 단계 (D-40) | **건너뛴 단계는 실패가 아니라서** 빌드가 계속 SUCCESS |
| 중복 회차 스킵 (D-41) | 테스트가 그 경로를 밟으면서 **행 개수만 검사** |
| Zipkin 사망 (D-44) | 헬스체크가 없어 `docker ps`에 정상으로 보임 |

전부 "실패하지 않았다"와 "성공했다"를 혼동한 것입니다. **무엇을 검사하지 않고 있는지**를 따로 묻지 않으면 드러나지 않습니다.

**기본값은 결정입니다.** `query-passing: false`(D-43), `MEM_MAX_SPANS: 500000`(D-44), `when { branch 'main' }`의 빈 `BRANCH_NAME`(D-40) — 셋 다 아무도 고르지 않았지만 시스템의 동작을 정했습니다. 특히 D-43은 요청의 절반을 죽이면서도 **애플리케이션 코드 버그처럼 보였습니다.**

**어휘가 잘못되면 호출 측이 잘못 씁니다.** D-41의 원인은 내부 멱등 스킵을 PG 응답 타입으로 표현한 것이었습니다. 타입이 "거절"이라고 말하니 호출 측이 거절로 다룬 것은 자연스러운 일이었습니다. 문자열 비교로 특수 처리하는 대신 타입을 갈랐습니다.
...
| 문서 | 내용 |
|---|---|
| [ARCHITECTURE.md](ARCHITECTURE.md) | 아키텍처 · 사가 흐름 · 가용성 로드맵 |
| [DECISIONS.md](DECISIONS.md) | 설계 결정 45건 + 미결 20건 |
