...
로그인만 하면 누구나 재생 URL을 받을 수 있었습니다 — 유료 강의가 사실상 무료였습니다.

**동기 호출을 넣지 않았습니다.** course가 재생마다 subscription을 부르면, 구독 생성보다 훨씬 잦은 경로가 subscription의 가용성에 묶입니다. D-17이 이벤트로 끊어둔 결합이 되살아납니다.

대신 subscription이 발행하는 이벤트를 course가 소비해 **"지금 볼 수 있는가"만 답하는 최소 사본**을 둡니다. 금액·플랜은 복제하지 않습니다 — 필요 없는 것을 복제하면 그것도 맞춰야 할 상태가 됩니다.

대가는 결과적 일관성입니다. 실측 지연은 구독·해지 양방향 모두 **1초 이내**였습니다.
