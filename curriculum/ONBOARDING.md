...
한 모델이 하루 한도(20건)를 다 쓰면 `GEMINI_MODELS` 에 적힌 다음 모델로 자동으로 갈아탄다.
**429(한도)일 때만 갈아탄다.** 5xx 나 네트워크 오류는 모델을 바꿔도 그대로라 그 자리에서 끝낸다.
어느 모델이 답했는지는 화면 오른쪽 위 배지와 응답의 `model` 필드에 나온다.

```
GEMINI_MODELS=gemini-3.1-flash-lite,gemini-3.6-flash,gemini-3.7-flash
```

비워두면 `GEMINI_MODEL` 하나만 쓴다.
