...
LangGraph 로 묶여 있다 (`roadmap/engine.py`). 기본 최대 3회 호출한다.
...
for 문으로도 같은 흐름이 돈다. 실제로 처음에는 for 문이었다. 묶은 이유는
사이클 자체가 아니라 **트레이스에 무엇이 남느냐**다.

`llm.generate` 에 데코레이터만 달면 이렇게 남는다.
...
재생성이 왜 일어났는지는 지역변수 `problems` 안에 있어서 트레이스 밖이다.
노드로 쪼개면 그게 그대로 올라간다.

```
generate  시도 1 · 토큰 in 2800
verify    실패: 기간 초과: 40h > 20h
generate  시도 2 · 토큰 in 5600
verify    통과
schedule  2주
```

아래 "첫 시도 통과율"이 프롬프트 품질 지표인데, 그 지표가 대시보드에서 보이려면
이 구조가 필요하다. 지금은 평가 러너를 손으로 돌려야만 나온다.

**안 쓰는 것도 적어둔다.** 체크포인터·`interrupt`·병렬 노드를 쓰지 않는다.
사람이 중간에 끼어들 지점이 없고 요청 하나 안에서 끝난다. 강의 생성 중복 체크
기능은 강사 판단을 기다리는 구간이 있어서 그때는 쓰게 된다.

`llm` 과 `on_retry` 는 상태가 아니라 `config` 로 넘긴다. 체크포인터를 붙이는 날
상태는 직렬화 대상이 되는데 소켓을 쥔 객체와 콜백은 직렬화가 안 된다.
...
LangGraph 로 옮긴 뒤 다시 돌렸다. 모델 `gemini-3.5-flash-lite`.

```
R-01  통과   24개 · 439h/520h · 트랙  75% · 채움  98%
R-02  통과    6개 · 133h/240h · 트랙 100% · 채움  55%
R-03  통과    6개 · 108h/240h · 트랙 100% · 채움  53%
R-04  통과    6개 · 116h/200h · 트랙 100% · 채움  58%
...
8/8 통과 · 첫 시도 통과 7/8 · 호출 9회 · 토큰 입력 17,211 / 출력 2,795
```

**통과 여부와 호출 횟수는 리팩터 전과 같다.** 개별 숫자가 흔들리는 것은
temperature 가 0 이 아니라서다(0.2). R-01 의 트랙 비율이 86%에서 75%로
내려갔지만 임계값 위다. 판정이 바뀐 케이스는 없다.
...
`ROADMAP_DEADLINE_SECONDS` (기본 180초). 안 걸면 이렇게 된다.

```text
재생성 3회 x 재시도 4회 x 대기 60초 + 백오프 = 최대 25분
```

gateway 와 브라우저는 그 전에 끊는데 **서버는 그것도 모르고 계속 모델을 부른다.**
쿼터만 태우고 아무도 안 받는다. 그래서 예산을 `llm.generate` 까지 내려보낸다 —
재시도를 멈추고, 남은 시간이 짧으면 그만큼만 기다린다.

시간이 다 되면 재생성을 포기하고 **지금까지 고른 것을 검증 문제와 함께** 내보낸다.
아무것도 안 주는 것보다 낫다.

CLI 는 예산을 안 건다. 오래 기다려도 되기 때문이다.
