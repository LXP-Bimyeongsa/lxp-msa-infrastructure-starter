...
OUT_OF_SCOPE     위 목록 어느 자료와도 이어지지 않는다

애매하면 OUT_OF_SCOPE 로 두지 말고 CONCEPT 으로 둔다. 뒤에서 실제로 검색해 보고
답할 내용이 있는지 다시 판정한다. 여기서 잘못 밀어내면 답할 수 있는 질문에
답하지 못하고 끝나지만, 잘못 통과시키면 뒤 단계가 받아낸다.

제목은 장 단위라 세부 내용까지 드러내지 않는다. 제목이 다루는 범위 안에 들어갈
법하면 이어진 것으로 본다
